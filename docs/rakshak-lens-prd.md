# Rakshak Lens — Product Requirements Document (PRD)
...
Rakshak Lens is an on-premise, AI-powered video analytics platform for industrial safety. It processes live camera feeds to detect safety violations and alerts in real-time.
...
**Deployment model: 100% on-premise.** All processing, storage, and alerting runs on the customer's edge device. No video data leaves the premises. The only internet connection required is a one-time license activation call to the Rakshak Lens licensing server.
...
The system must not run without a valid license. No one can deploy Rakshak Lens without explicit authorization.
...
│     └── Disk serial number                                          │
│                                                                     │
│  3. Device sends to Rakshak Lens License API:                         │
│     POST https://license.rakshaklens.ai/activate                    │
│     {                                                               │
...
| LIC-3 | License API server | Cloud API to validate and issue licenses | Hosted at `license.rakshaklens.ai`. Validates product key, binds to hardware, returns signed token. Admin panel to manage keys. | Not built |
| LIC-4 | Signed license token | Cryptographically signed license file | RSA-2048 signed by Jiffy Labs private key. Public key embedded in Rakshak Lens binary. Cannot be forged or modified. | Not built |
...
| Domain | `license.rakshaklens.ai` | Cloudflare | ~$10/year |
...
| Domain + VPS for license server | Blocks product key activation | Register `license.rakshaklens.ai`, set up VPS. Low cost (< $15/month). |
