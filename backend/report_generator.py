"""
PDF compliance report generator for SafetyLens.
...
logger = logging.getLogger("safetylens.reports")
...
def _add_footer(canvas, doc):
    ...
    canvas.drawString(20 * mm, 10 * mm, f"Generated by SafetyLens — Page {doc.page}")
...
def _email_report(pdf_bytes: bytes, recipients: list[str], start: str, end: str) -> None:
    ...
    msg = MIMEMultipart()
    msg["Subject"] = f"[SafetyLens] Safety Report {start} to {end}"
    msg["From"] = from_addr
    msg["To"] = ", ".join(recipients)

    body = MIMEText(
        f"<h3>SafetyLens Safety Compliance Report</h3>"
    ...
    attachment.add_header("Content-Disposition", "attachment", filename=f"safetylens-report-{start}-to-{end}.pdf")
