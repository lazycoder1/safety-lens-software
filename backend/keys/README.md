...
This folder holds the **public key** used by Rakshak Lens edge to verify signed license files and heartbeat tokens issued by License Hub.
...
- **Developer machine (Gautham):** `~/.rakshak-lens/license_private.pem` (mode 600)
- **Production:** Vercel encrypted env var `LICENSE_SIGNING_KEY` on the License Hub deployment
- **Backup:** Sealed copy in 1Password vault `Techser → Rakshak Lens → License Signing Key`
...
2. Ship new public key in next Rakshak Lens release
...
The private key goes to `~/.rakshak-lens/license_private.pem` (chmod 600). The public key goes here.
