...
def test_production_delivery_adapter_preserves_per_channel_outcomes(monkeypatch):
    ...
    def notify(_alert, _snapshot, *, output_ids):
        calls.append(output_ids)
        return [
            {"outputId": "inApp", "type": "in_app", "status": "delivered"},
            {"outputId": "email", "type": "email", "status": "delivered"},
            {"outputId": "telegram", "type": "telegram", "status": "failed"},
            {"outputId": "sms", "type": "sms", "status": "skipped"},
        ]

    monkeypatch.setattr(video_processing.notification_dispatcher, "notify", notify)
