...
def main():
    ...
    detector_model = yolo.get("model") or "yolo26n"
...
| `{detector_model}` | Object Detection (YOLO26) | nano weights | Fine-tuned real-time detection |
...
"""
    ...
    if yolo and "metrics" in yolo:
        ...
        m = yolo["metrics"]
        inf = yolo["inference"]
        md += f"""**Model:** {detector_model} (nano)
**Dataset:** Auto-labeled from test videos using YOLOE-26 promptable detection
...
| Model | FPS | Use Case |
|-------|-----|----------|
| **{detector_model} (fine-tuned)** | ~{yolo_fps} | Real-time video analytics, edge deployment |
...
| Capability | {detector_model} | qwen3-vl:8b | qwen3.5:35b |
|-----------|---------|------------|------------|
...
| Novel classes (no training) | Assisted by YOLOE-26 labels before training | ✅ | ✅ |
